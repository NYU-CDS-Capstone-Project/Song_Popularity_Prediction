...
MEL_BIN = 128
FRAME_NUM = 969 #30s is 323， 90s is 969
TAG_SIZE = 50
A2V_SIZE = 40
DR = 0.25
LR = 5e-3
...
def make_batch(feat, pt, trix, pos, b_size): #trix train index,pos
    ids = trix[pos:pos + b_size]
...
def inception_cnn(x, dr, mode, stt):
    ...
    # 1*320*64
    concat = tf.concat([conv1_128_4, conv1_132_8, conv1_140_16], axis=3)  #inception module without maxpooling???, maybe added it?
    # 1*320*32
...
def cnn_body(head, dr, mode, stt):
    ...
    if 't' in stt:  #trend, second target
        # 1*78*TIMESPAN
...
def main():
    ...
    parser.add_argument(
        '-ep', '--ep_num', help='episode num for training', default=1
    )
    parser.add_argument(
    ...
    # feat:    3D, data_num * MEL_BIN * FRAME_NUM   , what is FRAME_NUM
    # clss:    1D, data_num (int; 1:clsn)
    # ids:     1D, data_num (int)
    # trd_max: 1D, data_num (float)
    # trd:     2D, data_num * time_len (float)
    feat = np.load(
        path.join(train_set, 'feat.npy')).reshape(-1, MEL_BIN, FRAME_NUM, 1) #actually here is (data_num * MEL_BIN * FRAME_NUM*1)
    #FRAME_NUM,就是之前mel_extract.py 的 mel spectrum,  shape is (1,128,feat_len)  里面的feat_len
    ids = np.load(path.join(train_set, 'id.npy')) #[ data_num,],just song ids, name? 'id.feat.npy'
    # trd = np.load(path.join(train_set, 'trend.feat.npy'))  # maybe not used in training for capstone project
    #这个就是要预测的target
    ret = np.load(path.join(train_set, 'rank.npy')) # in a time_len, the ranking numbers, maybe not used in training for capstone project
    ...
    tr_val = floor(data_num * (TRAIN_SIZE + VAL_SIZE)) #the largest integer value less than or equal to x.
    ...
    train_ret_30 = train_ret.reshape(-1, 1) #np.mean(train_ret, axis=1).reshape(-1, 1) #[data_num * 1] #averages of streams
    # test_ret_30 = test_ret.reshape(-1, 1) #np.mean(test_ret, axis=1).reshape(-1, 1)

    # timespan = train_trd.shape[1]
    # ret_timespan = None
    train_pt, test_pt, y_size = None, None, 1
    train_pt = train_ret_30 #ranking targets
    # test_pt = test_ret_30
    ...
    if use_tag:
        ...
        all_tags = np.load(path.join(train_set, 'tgtr.{}.npy'.format(tag_type)))
        train_tags = np.take(all_tags, trix, axis=0)
        # test_tags = np.take(all_tags, teix, axis=0)
    ...
    if use_tag:
        logits_tag = tag_regression_clsf(
            tags, dr_rate, mode)  # convoluted tags
    if use_a2v:
        logits_a2v = a2v_regression_clsf(
            a2v, dr_rate, mode)
