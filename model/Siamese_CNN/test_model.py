...
if __name__ == '__main__':
    ...
    #threshold setting
    tag_tr_fp = 'tag_premodel/data.magnatagatune/tag_list.top188.txt'
    tag_te_fp = 'tag_premodel/data.magnatagatune/tag_list.top50.txt'

    tag_idx_list = utils.get_test_tag_50(tag_tr_fp, tag_te_fp)

    model_dir = 'tag_premodel/models'
    ...
    # Files
    param_fp = 'tag_premodel/models/model.20160309_111546.npz'
    tag_tr_fp = 'tag_premodel/data.magnatagatune/tag_list.top188.txt'
    tag_te_fp = 'tag_premodel/data.magnatagatune/tag_list.top{}.txt'.format(n_top_tags_te)
    ...
    # Load data
    print("Loading data...")
    X_te_list.append(np.load(data_dir +'/feat_train_512.npy'))
    X_te_list.append(np.load(data_dir +'/feat_train_1024.npy'))
    X_te_list.append(np.load(data_dir +'/feat_train_2048.npy'))
    X_te_list.append(np.load(data_dir +'/feat_train_4096.npy'))
    X_te_list.append(np.load(data_dir +'/feat_train_8192.npy'))
    X_te_list.append(np.load(data_dir +'/feat_train_16384.npy'))
    ...
    pred_all_raw = np.vstack(pred_list_raw) #combine tags of 6 different windows

    pred_all = pred_all_raw[:, label_idx_list]
    pred_binary = ((pred_all-thresholds) > 0).astype(int)

    np.save('train_data/tgtr.jynet.npy', pred_all)
